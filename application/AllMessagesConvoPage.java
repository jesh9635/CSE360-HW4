...
/**
 * AllMessagesConvoPage handles showing messages between two people. If it is accessed
 * through the AllMessagesUsersPage, it only shows the message. If it is accessed through
 * the ElevatedMessageUsersPage, then it also allows for typing and submitting a post.
 */
public class AllMessagesConvoPage {
	
    private final DatabaseHelper databaseHelper;
    private final User currentUser;
    
    /**
     * Initializes the AllMessagesConvoPage with the database helper and logged in user
     *
     * @param databaseHelper The helper class for interfacing with the database.
     * @param user The user who is currently logged in.
     */
    public AllMessagesConvoPage(DatabaseHelper databaseHelper, User user) {
    ...
    public void show(Stage primaryStage, User otherUser, boolean elevated) {
        ...
        if (elevated) {
	        ...
	        submitMessageButton.setOnAction(e -> {
	            ...
	            if (!messageText.isEmpty()) {
	                PrivateMessage p = new PrivateMessage(messageText, currentUser.getUserName(), otherUser.getUserName(), 1, 0, false);
	                // Add question to database
	                try {
						if (databaseHelper.createPrivateMessage(p)) {
						    // Refresh the page to show the new question
						    show(primaryStage, otherUser, true);
						}
     ...
     *
     * @param privateMessage The privateMessage to display.
